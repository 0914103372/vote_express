...
const createOption = async (option)=>{
    ...
    const createOption = await connection.query(query,[option.content,option.createdAt,option.poll_id])
    .catch(err=>{
        console.error('loi create option',err)
    })
...
const getOption = async (option_id)=>{
    ...
    const getOption = await connection.query(query,[option_id])
    .then(data=>{
        console.log(data[0])
        return data[0]
    })
...
const update = async (option)=>{
    ...
    const update = await connection.query(query,[option.content,option_id])
    .then(()=>{
...
const deleteOption = async (option_id)=>{
    ...
    const deletepoll = await connection.query(query,[option_id])
    .then(()=>{
